...
const end = () => items.forEach((item) => item.classList.remove('active','grabbing'));
...
function activate(e) {
  ...
  const insertPos = inactiveItems.find((item) => e.clientY <= item.offsetTop + (item.offsetHeight/2));
